...
registry.registerPath({ method: "get", path: "/api/v1/crm/site-settings", summary: "List site settings, optionally filtered by group (small, non-paginated: exactly the 7 seeded keys)", tags: ["crm", "site-settings"], security: [{ cookieAuth: [] }], ...requiredPermission("site_settings.view"), request: { query: ListSiteSettingsQuerySchema }, responses: { 200: { description: "Site setting list.", content: { "application/json": { schema: SiteSettingListEnvelope } } }, ...errorResponses } });
...
registry.registerPath({ method: "put", path: "/api/v1/crm/site-settings/{key}", summary: "Upsert a site setting's value (server validates `value` against that key's closed shape from SiteSettingValueSchemaByKey; 404 for any key outside the 7 seeded literals)", tags: ["crm", "site-settings"], security: [{ cookieAuth: [], csrfHeader: [] }], ...requiredPermission("site_settings.edit"), request: { params: z.object({ key: SiteSettingKeySchema }), body: { content: { "application/json": { schema: UpsertSiteSettingRequest } } }, headers: idempotencyKeyHeader }, responses: { 200: { description: "Site setting set.", content: { "application/json": { schema: SiteSettingEnvelope } } }, ...errorResponses } });
registry.registerPath({ method: "get", path: "/api/v1/public/site-settings", summary: "Get all 7 sitewide settings values (anonymous, cacheable — nav/footer/SEO/contact)", tags: ["public", "site-settings"], responses: { 200: { description: "All site settings.", content: { "application/json": { schema: PublicSiteSettingsEnvelope } } }, ...errorResponses } });
